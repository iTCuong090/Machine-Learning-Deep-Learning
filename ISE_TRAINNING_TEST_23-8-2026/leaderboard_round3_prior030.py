"""Leaderboard iteration 3: change prior alpha only, 0.35 -> 0.30."""
...
NEW_ALPHA = 0.30
GAMMA = 0.75
OUTPUT = ROOT / "submission_iter3_prior_alpha_0p30.csv"


def adjusted_oof(
    blend: np.ndarray, y: np.ndarray, fold: np.ndarray, alpha: float
) -> np.ndarray:
    result = np.empty_like(blend)
    for held_out in range(N_SPLITS):
        valid = fold == held_out
        count = np.bincount(y[~valid], minlength=N_CLASSES).astype(float)
        prior = count / count.sum()
        values = blend[valid] / np.power(prior[None, :], alpha)
        result[valid] = values / values.sum(axis=1, keepdims=True)
    return result


def crossfold_scores(
    y: np.ndarray, fold: np.ndarray, probability: np.ndarray
) -> list[float]:
    scores = []
    for held_out in range(N_SPLITS):
        valid = fold == held_out
        thresholds = fit_thresholds(y, probability, ~valid)
        prediction = (
            probability[valid] / np.power(thresholds[None, :], GAMMA)
        ).argmax(axis=1)
        scores.append(macro_f1(y[valid], prediction))
    return scores
...
def main() -> None:
    ...
    report = {
        ...
        "rows_changed_vs_round2": int(changed.sum()),
        "fraction_changed_vs_round2": float(changed.mean()),
    ...
    (ARTIFACT_DIR / "round3_submission_report.json").write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )

    history_path = ARTIFACT_DIR / "leaderboard_history.csv"
    history = pd.read_csv(history_path)
    history.loc[history["round"] == 2, ["public_macro_f1", "status"]] = [
        0.402,
        "scored",
    ]
    history = pd.concat(
                ...
                [
                    {
                        ...
                        "single_change": "prior_alpha 0.35 -> 0.30",
    ...
    history.to_csv(ARTIFACT_DIR / "leaderboard_history_after_round2.csv", index=False)
